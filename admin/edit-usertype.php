...
if(!isset($_GET['eid'])){
   header("location:display-usertype.php");
}
$eid = $_GET['eid'];

$edit_query = mysqli_query($conn, "select * from tbl_usertype where User_type_id='{$eid}'");
$edit_details = mysqli_fetch_array($edit_query);
  ...
  $qry = "update `tbl_usertype` set `User_type`='{$User_type}',`company_code`='{$company_code}',`is_active`='{$is_active}',`is_delete`='{$is_delete}',`insert_datetime`='{$insert_datetime}' where User_type_id='{$eid}'";
   ...
   if($result){
     echo "<script>alert('Record updated');window.location='display-usertype.php';</script>";
  ...
  <body>
    <div class="be-wrapper">
      ...
      <div class="be-content">
        <div class="page-head">
          <h2 class="page-head-title">EDIT usertype</h2>
          <ol class="breadcrumb page-head-nav">
            <li><a href="#">Home</a></li>
            <li><a href="#">Forms</a></li>
            <li class="active">edit usertype</li>
          </ol>
        </div>
              ...
              <div class="panel panel-default panel-border-color panel-border-color-primary">
                ...
                <div class="panel-body">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-6">
                          <input type="text"  value="<?php echo $edit_details['User_type']; ?>" placeholder="Enter User type" class="form-control" name="User_type">
                      </div>
                    </div>
                      ...
                      <div class="col-sm-8">
                        <?php
                        $sql = mysqli_query($conn,"SELECT * From  tbl_productcompany") or die(mysqli_error($conn));
                         $row = mysqli_num_rows($sql);
                        echo '<select name="company_code"  class="form-control">';
                             while ($row = $sql->fetch_array())
                                        {
                                             echo "<option value='". $row['company_code'] ."'>" .$row['company_code'] ."</option>" ;
                                        }
                         echo "</select>" ;
                        ?>
                    </div>
